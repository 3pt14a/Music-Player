...
document.getElementById("profileIcon").addEventListener("click", function() {
    ...
    var mp3Content = document.querySelector(".box-container"); // Select the mp3 content container
    
    form.classList.toggle("show");
    mp3Content.classList.toggle("blur"); // Toggle blur on the mp3 content
});

// Close the login form when clicking outside of it
window.addEventListener("click", function(event) {
    var form = document.getElementById("loginForm");
    var mp3Content = document.querySelector(".box-container");

    // Check if the form is shown and the clicked target is outside the form
    if (form.classList.contains("show") && !form.contains(event.target) && event.target.id !== "profileIcon") {
        form.classList.remove("show");
        mp3Content.classList.remove("blur"); // Remove blur when form is closed
    }
});
